...
forms.forEach( elem => {
    elem.onsubmit = () => {
        ...
        fieldsToVal = {
            name: elem.querySelector('input[name="ur-name"]'),
            phone: elem.querySelector('input[name="ur-phone"]'),
            city: elem.querySelector('input[name="ur-city"]'),
        }

        for (let field in fieldsToVal) {
            if(!fieldsToVal[field].value) {
                fieldsToVal[field].closest('.popup__form-field').classList.add('not-valid')
            }
        }

        if (elem.querySelector('.not-valid')) {
            elem.classList.add('has-invalid-fields')
            elem.querySelectorAll('.not-valid').forEach( input=>{
                input.onclick = ()=>{
                    input.classList.remove('not-valid')
                    if(!elem.querySelector('.not-valid')) {
                        elem.classList.remove('has-invalid-fields')
                    }
                }
            })
            return false;
        } else {
            elem.querySelector('.form-submit').setAttribute('disabled', true)
        }
